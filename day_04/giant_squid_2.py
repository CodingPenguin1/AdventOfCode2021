...
if __name__ == '__main__':
    ...
    turns_taken = [0] * len(boards)
    for i, board in enumerate(boards):
        for turn_counter, number in enumerate(numbers):
            board.mark(number)
            if board.check_win():
                turns_taken[i] = turn_counter
                break

    worst_board = turns_taken.index(max(turns_taken))
    print(boards[worst_board].get_score())
