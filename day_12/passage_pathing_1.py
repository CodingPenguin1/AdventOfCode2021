...
def navigate(current_cave_index, visited, current_path, depth):
    ...
    if not current_cave.big:
        visited[current_cave_index] = True
    ...
    for i, cave in enumerate(caves):
        ...
        if cave.name != 'start':
            ...
            if cave.name != current_cave.name:
                # If it's a small cave, make sure we haven't already visited it
                if cave.big or (not cave.big and not visited[i]):
...
if __name__ == '__main__':
    ...
    for i, cave in enumerate(caves):
        if cave.name == 'start':
            start_index = i
            break
    navigate(start_index, [False] * len(caves), [], 0)
    print(len(paths))
